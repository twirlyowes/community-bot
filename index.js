...
const commands=[require("./commands/ping"),require("./commands/help"),require("./commands/config"),warningCommand];
...
client.once(Events.ClientReady,async ready=>{
  ...
  try{
    ...
    await rest.put(Routes.applicationCommands(ready.user.id),{body:commands.map(c=>c.data.toJSON())});
...
client.on(Events.InteractionCreate,async i=>{
  ...
  if(!cmd) return;
  try{await cmd.execute(i,await getGuildConfig(i.guildId));}
  catch(e){
    console.error(e);
    const p={content:"Something went wrong.",ephemeral:true};
...
client.on(Events.MessageCreate,async m=>{
  ...
  try{
    ...
    const n=m.content.slice(c.prefix.length).trim().split(/\s+/)[0]?.toLowerCase();
    if(n==="ping") return m.reply(card(c,"Pong!","WebSocket latency: **"+client.ws.ping+"ms**","Ping"));
    if(n==="help") return m.reply(card(c,"Help","`"+c.prefix+"ping` — Check latency\n`"+c.prefix+"help` — Show help","Help"));
  }catch(e){console.error("Message command failed:",e);}
});

warningCommand.registerPrefix(client);
