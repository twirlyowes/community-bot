...
const DEFAULT_CONFIG={prefix:".",branding:{name:"Community Bot",footer:"Community Bot",color:0x38BDF8},moderation:{staffRoleId:null,logChannelId:null}};
...
async function getGuildConfig(id){const r=ref(id),s=await r.get();if(!s.exists){const c=clone();await r.set(c);return c;}return merge(DEFAULT_CONFIG,s.data());}
async function updateGuildConfig(id,changes){await ref(id).set(changes,{merge:true});return getGuildConfig(id);}
module.exports={DEFAULT_CONFIG,getGuildConfig,updateGuildConfig,initializeGuild:async id=>getGuildConfig(id)};