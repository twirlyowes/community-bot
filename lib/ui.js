...
function createCard({
  ...
  sections.forEach((section, index) => {
    const text = new TextDisplayBuilder().setContent(section);
...
function card(config, title, body, feature = title) {
  const content = `## ${title}\n\n${body}\n\n${footer(config, feature)}`;
  return messageOptions(createCard({
    color: config?.branding?.color || COLORS.SKY_BLUE,
    content
  }));
}
